...
async def get_analytics():
    try:
        ...
        error_count = sum(a.get('error_count', 0) for a in accounts)
        
        # Get pool health report
...
async def start_parsing(req: ParseRequest):
    ...
    if not all([settings.telegram_api_id, settings.telegram_api_hash]):
        raise HTTPException(
            status_code=503, 
            detail="Telegram credentials not configured. Set TELEGRAM_API_ID and TELEGRAM_API_HASH in .env.local"
        )
    ...
    try:
        ...
        with sqlite3.connect(tasks_db) as conn:
            ...
            conn.execute(
                ...
                (task_id, req.parser_type, "processing", json.dumps(req.dict()), datetime.now().isoformat(), None, None)
            )
            conn.commit()
        
        # Initialize Telegram client with shorter timeout
        telegram = TelegramUserClient(
            api_id=settings.telegram_api_id,
            api_hash=settings.telegram_api_hash,
            phone=settings.telegram_phone,
            session_path="data/sessions/api_task.session"
        )
        
        # Connect with timeout
        try:
            connected = await asyncio.wait_for(telegram.connect(), timeout=15.0)
        except asyncio.TimeoutError:
            raise HTTPException(status_code=504, detail="Connection to Telegram timed out")
        
        if not connected:
            raise HTTPException(status_code=503, detail="Failed to connect to Telegram")
        
        try:
            discovery = ChannelDiscovery(telegram)
            
            if req.parser_type == "keywords":
                keywords = req.keywords.split(",") if req.keywords else [req.target]
                results = await discovery.search_by_keywords(keywords, limit=min(req.limit, 50))
                results = await discovery.filter_open_comments(results)
            elif req.parser_type == "channels":
                results = await discovery.search_by_keywords([req.target], limit=min(req.limit, 50))
            else:
                results = await discovery.search_by_keywords([req.target], limit=min(req.limit, 50))
            ...
            with sqlite3.connect(tasks_db) as conn:
                conn.execute(
                    ...
                    ("completed", datetime.now().isoformat(), json.dumps(results), task_id)
                )
                conn.commit()
...
# Global orchestrator instance for API sessions
_api_orchestrator: PipelineOrchestrator = None
_api_telegram: TelegramUserClient = None

@app.post("/api/v1/commenting/start")
async def start_commenting(config: CommentingConfig):
    global _api_orchestrator, _api_telegram
    ...
    try:
        # Initialize or reconnect Telegram client with timeout
        if _api_telegram is None or not _api_telegram.is_connected:
            _api_telegram = TelegramUserClient(
                api_id=settings.telegram_api_id,
                api_hash=settings.telegram_api_hash,
                phone=settings.telegram_phone,
                session_path="data/sessions/api_pipeline.session"
            )
            try:
                connected = await asyncio.wait_for(_api_telegram.connect(), timeout=15.0)
            except asyncio.TimeoutError:
                raise HTTPException(status_code=504, detail="Connection to Telegram timed out")
        ...
        # Create components
        comment_sender = CommentSender(_api_telegram)
        
        # Create and start orchestrator
        session_id = f"session_{uuid.uuid4().hex[:12]}"
        _api_orchestrator = PipelineOrchestrator(
            telegram_client=_api_telegram,
            comment_sender=comment_sender,
            settings={"work_mode": config.tone}
        )
        
        # Start in background
        asyncio.create_task(_api_orchestrator.start(
            target_channels=config.channels,
            style=config.tone,
            max_comments_per_hour=10
        ))
        
        # Wait a moment to check if started
        await asyncio.sleep(1)
        
        return {
            "session_id": session_id,
            "status": "running" if _api_orchestrator.is_running else "failed_to_start",
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start commenting: {str(e)}")
...
# --- Crisis Manager Endpoints ---
class CrisisReportRequest(BaseModel):
    account_id: str
    incident_type: str  # flood_error, report_received, suspicious_activity, etc.
    details: Optional[str] = None


@app.post("/api/v1/crisis/report")
    ...
    try:
        ...
        if crisis_result:
            return {
                ...
                "analysis": crisis_result.get("analysis", ""),
                "options": crisis_result.get("options", [])
            }
        else:
            return {
                "account_id": req.account_id,
        ...
        raise HTTPException(status_code=500, detail=f"Crisis report error: {str(e)}")
